...
// This script tests the token refresh system
...
async function testTokenRefresh() {
  ...
  try {
    ...
    if (!profileResponse.ok) {
      throw new Error(`Profile access failed: ${profileResponse.status} ${profileResponse.statusText}`);
    }
    ...
    if (!newProfileResponse.ok) {
      throw new Error(`Profile access with new token failed: ${newProfileResponse.status} ${newProfileResponse.statusText}`);
    }
    ...
    // Step 7: Test that refresh token is invalidated
    console.log('7️⃣ Testing that refresh token is invalidated after logout...');
...
async function runAllTests() {
  ...
  console.log('\n✨ All token refresh and expiration tests completed successfully!');
}
